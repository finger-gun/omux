...
"cmd+shift+n" = "workspace.restore-last-closed"
"cmd+shift+backspace" = "workspace.close"
...
| `workspace.restore-last-closed`             | Restore the most recently closed workspace.                         |
...
Default bindings are generated by `omux config init`. In the palette, a first-character `>` switches to command search; whitespace before `>` remains workspace search text.
