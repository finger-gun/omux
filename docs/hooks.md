...
For multi-vendor AI/tool workflows, prefer delegating the matching logic to the installable `ai-status` plugin host from the official plugin registry instead of hardcoding vendor-specific title or transcript parsing into every hook script. Installed plugins can now declare their own hook callbacks in `omux-plugin.toml`, so the official `ai-status` package observes Codex title changes without requiring a user-installed forwarding script.
